...
def findcorruptedfiles(input_dir: str, samplelist: list[str]):
    """ Function to identify which npz files in a list have been corrupted
        ...
        Returns:
            corrupted (list[str]): list of npz files that were found to be corrupted
    ...
    print(len(corrupted), 'corrupted samples found out of', len(samplelist), 'files tested.')

    return corrupted


def maketvtlists(search_dir: str, sample_split: tuple[float, float, float], save_path: NoneStr=None, save: bool=True):
    """ Function to make training, validation, and testing samples lists and save them to files

        Args:
            search_dir (str): file path to directory to search for samples in; include any restrictions for file name
            sample_split (tuple[float, float, float]): training, validation, and testing split percentages;
                                                        must sum to 1.0;
                                                        to create one list containing all samples, use (1, 0, 0)
        ...
        Returns:
            train_samples (list[str]): list of samples corresponding to sample_split[0] fraction of total samples; if save=True, will be saved to .txt file
            val_samples (list[str]): list of samples corresponding to sample_split[1] fraction of total samples; if save=True, will be saved to .txt file
            test_samples (list[str]): list of samples corresponding to sample_split[2] fraction of total samples; if save=True, will be saved to .txt file

    """
    ## Test Sample Split
    assert sum(sample_split) <=1, "Sum of training, validation, and testing split must be less than or equal to 1.0"

    ## Gather Samples
    sample_list = glob.glob(search_dir)
    sample_list = np.unique(sample_list).tolist()
    corrupted = findcorruptedfiles(input_dir=os.path.dirname(search_dir), samplelist=sample_list)
    ...
    if save:
        if save_path == None:
            raise ValueError('None is not a valid save path for makefilelist. Either provide a valid save path or use save=False.')
        else:
            if train > 0:
...
if __name__ == '__main__':
    ...
    ## Make File Lists
    ########################
    train_samples, val_samples, test_samples = maketvtlists(search_dir=os.path.join(input_dir, data_tag), 
