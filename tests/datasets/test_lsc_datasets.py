...
def dataset() -> LSC_rho2rho_temporal_DataSet:
    ...
    with patch("builtins.open", mock_open(read_data=mock_file_list)):
        with patch("random.shuffle") as mock_shuffle:  # Track if shuffle is called
...
def test_dataset_init(dataset: LSC_rho2rho_temporal_DataSet) -> None:
    ...
    assert dataset.max_file_checks == 10
...
@patch("pathlib.Path.is_file", return_value=True)  # Mock file existence check
def test_getitem(
...
def test_getitem_max_file_checks(
    ...
    """
    error_str = "Two files were not found within " "max_file_checks attempts."
    with pytest.raises(FileNotFoundError, match=error_str):
        dataset[0]
