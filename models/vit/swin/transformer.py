...
from models.vit.swin.encoder import SwinEncoder
from models.vit.patch_embed import SwinEmbedding
from models.vit.patch_manipulation import PatchMerging


class Swin(nn.Module):
    """Main SWIN architecture. This corresponds to the `SWIN tiny` architecture
    in the original 2021 SWIN paper. This takes in an image and predicts a
    vector of size `num_output_classes`.
    ...
    def __init__(self,
                 ...
                 emb_factor: int=2,
                 num_heads: int=10,
                 window_sizes: [(int, int), (int, int), (int, int), (int, int)]=[(8, 4), (8, 4), (8, 4), (8, 4)],
        ...
        self.stage2 = SwinEncoder(emb_size=new_emb_size,
        ...
        self.stage3 = nn.ModuleList([SwinEncoder(emb_size=new_emb_size,
                                                 num_heads=new_num_heads,
                                                 patch_grid_size=new_patch_grid_size,
                                                 window_size=self.window_sizes[2]),
                                     SwinEncoder(emb_size=new_emb_size,
                                                 num_heads=new_num_heads,
                                                 patch_grid_size=new_patch_grid_size,
                                                 window_size=self.window_sizes[2]),
                                     SwinEncoder(emb_size=new_emb_size,
        ...
        self.stage4 = SwinEncoder(emb_size=new_emb_size,
                                  num_heads=new_num_heads,
                                  patch_grid_size=new_patch_grid_size,
...
if __name__ == '__main__':
    ...
    # With embedding factor 2, the number of parameters is ~0.5 billion. With
    # embedding factor 4, the number of parameters is ~21.6 billion.
    model = Swin(input_channels=x.shape[1],
    ...
    print('SWIN output shape:', model(x).shape)
    print('Number of parameters:', count_torch_params(model, trainable=True))
