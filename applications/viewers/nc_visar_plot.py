...
class Visar:
    ...
    def plotVelTH(self, channels):
        ...
        for idx in channels:
            ...
            plt.plot(time, velocity, linewidth=1.7, label=f"Velocity, channel {idx:03d}")
    ...
    def plotDistTH(self, channels):
        ...
        for idx in channels:
            ...
            start_times.append(time[0])
            stop_times.append(time[-1])
            plt.plot(time, distance, linewidth=1.7, label=f"Distance, channel {idx:03d}")
...
parser = argparse.ArgumentParser(prog="Plot PDV output", description=descr_str)

# run index
parser.add_argument(
    "--runID", "-R", action="store", type=str, default="runID", help="Run identifier."
)
...
if __name__ == "__main__":
    ...
    filename = os.path.join(indir, f"{runID}_pdv.npz")
    ...
    if SAVEFIG:
        # Save figure
        fig1.savefig(f"{runID}_PDV.png", bbox_inches="tight")
    else:
        plt.show()
