...
def compute_CJ(
    ...
    def ps(vs: float) -> float: A * np.exp(-R1 * vs) + B * np.exp(-R2 * vs) + C / vs**k
    # adiabatic gamma on reference isentrope: gs(v/v0)
    def gs(vs: float) -> float:
        (vs * (A * R1 * np.exp(-R1 * vs) + B * R2 * np.exp(-R2 * vs) + C * k / vs**kp1)
         / ps(vs))
    # at CJ: vj/v0 = gj/(gj+1) assuming p0=0
    def fzero(vs: float) -> float: gs(vs) * (vs - 1) + vs
