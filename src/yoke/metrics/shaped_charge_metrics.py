...
class SCmetrics:
  ...
  def compute_volume(self):
    ...
    volume = np.matmul(np.reshape(height,(len(height),1)),np.reshape(surf_area,(1,len(surf_area))))
    return volume
  ...
  def plotField(self):
    ...
    img1 = ax1.imshow(field,
                      aspect='equal',
                      extent=[-self.Rcoord.max(),
                               self.Rcoord.max(),
                               self.Zcoord.min(),
                               self.Zcoord.max()],
                      origin='lower',
    ...
    fig1.savefig(os.path.join('png',self.filename+'.density.png'),bbox_inches='tight')
